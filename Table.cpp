 ...
 */
void Table::createVAO(int programID)
{
    // Cube vertex data
...
Table::Table( float width, float length, int id)
{
    ...
    this->legX = width / 8;
    this->legY = tableY * 0.8f;
    this->legZ = width / 8;

    this->topY = tableY * 0.2f;
...
void Table::render(int programID)
{
    ...
    // First the table top
	model = glm::scale( model, glm::vec3(tableX, topY, tableZ) );    
    ...
    // leg 1
    model = glm::mat4();
    model = glm::translate( model, glm::vec3(tableX-legX, -topY-legY, tableZ-legZ) );
